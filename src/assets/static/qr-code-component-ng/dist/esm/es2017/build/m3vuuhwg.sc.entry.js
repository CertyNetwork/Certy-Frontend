...
import { h } from '../bp-qr-code.core.js';
...
const cancel$1 = (model, _data, ctx) => {
...
const update$1 = (model, _data, ctx) => {
    if (model.players === _) {
        setup(model);
    }
...
const propsPlugin = {
    ...
    animate(effect) {
        ...
        const interpolate$$1 = interpolator(effect.to - effect.from, effect.keyframes);
...
const transformAngles = csvToList('rotateX,rotateY,rotateZ,rotate');
const transformScales = csvToList('scaleX,scaleY,scaleZ,scale');
const transformLengths = csvToList('perspective,x,y,z');
...
function on$1(element, event, listener) {
...
function init() {
    ...
    play$1 = id('ja-play');
    pause$1 = id('ja-pause');
    reverse$1 = id('ja-reverse');
    scrubberValue.value = '0';
    const scrubberChanged = (evt) => {
        var value = +evt.currentTarget.value;
        timeline$1.currentTime = value;
        onValueChanged(value);
    };
    on$1(scrubberControl, 'input', scrubberChanged);
    on$1(scrubberControl, 'change', scrubberChanged);
    on$1(scrubberValue, 'mousedown', () => {
        if (timeline$1) {
            timeline$1.pause();
        }
    });
    on$1(scrubberControl, 'mousedown', () => {
        if (timeline$1) {
            if (timeline$1.state === 3) {
                pausedForScrubbing = true;
            }
            timeline$1.pause();
        }
    });
    on$1(scrubberControl, 'mouseup', () => {
        if (timeline$1 && pausedForScrubbing) {
            pausedForScrubbing = false;
            timeline$1.play();
        }
    });
    on$1(scrubberValue, 'input', (evt) => {
        var value = +evt.currentTarget.value;
        timeline$1.currentTime = value;
        onValueChanged(value);
    });
    ...
    rates.forEach((rate) => {
        on$1(rate, 'click', () => {
            ...
            if (timeline$1) {
                const sign = timeline$1.playbackRate < 0 ? -1 : 1;
                timeline$1.playbackRate = +rate.value * sign;
            }
        });
    });
}
function player(timeline2) {
    ...
    if (timeline$1) {
        timeline$1.off('update', onValueChanged);
    }
...
class BpQRCode {
    ...
    executeAnimation(animation) {
        ...
        const timeline$$1 = animate(animationAdditions);
        player(timeline$$1);
        timeline$$1.play();
    }
    generateQRCodeSVG(contents, maskCenter) {
        const qr = qrcode_1(0, 'H');
        qr.addData(contents);
        qr.make();
        const margin = 4;
    ...
    ${renderQRModulesSVG(qr, this.moduleCount, margin, maskCenter, this.maskXToYRatio, this.legacy, this.moduleColor, coordinateShift)}
    </svg>`;
      ...
      `;
        ...
        function renderQRModulesSVG(qr, count, margin, maskCenter, maskXToYRatio, legacy, moduleFill, coordinateShift) {
            let svg = '';
    ...
    render() {
        return (h("div", { id: "qr-container" },
            h("div", { id: "icon-container", style: this.legacy ? { display: 'none', visibility: 'hidden' } : {} },
    ...
    static get is() { return "bp-qr-code"; }
    ...
    static get style() { return ".sc-bp-qr-code-h{display:block;contain:content}#qr-container.sc-bp-qr-code{position:relative}#icon-container.sc-bp-qr-code{position:absolute;width:100%;height:100%;display:-webkit-box;display:-ms-flexbox;display:flex;-webkit-box-align:center;-ms-flex-align:center;align-items:center;-webkit-box-pack:center;-ms-flex-pack:center;justify-content:center}"; }
}

export { BpQRCode as BpQrCode };
